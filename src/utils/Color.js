...
export class Color {
  ...
  get _min() {
    return Math.min(this._channels.r, this._channels.g, this._channels.b);
  }
  get _max() {
    return Math.max(this._channels.r, this._channels.g, this._channels.b);
  }
  get _delta() {
  ...
  get isValid() {
    return (
      isNumber(this._channels.a) &&
      isNumber(this._channels.r) &&
      isNumber(this._channels.g) &&
      isNumber(this._channels.b)
    );
  }
  /**
   ...
   */
  get v() {
    return Math.round(((this._max / 255) * 1000) / 10);
  }
  /**
      ...
      for (var i = 0; i < 3; i++) {
        if (channels[i]) {
          ...
          if (percent) {
            ...
            channels[i] =
              channels[i].search(/[^\d.\-%]/) === -1 && (!points || points.length < 2)
